...
namespace DBAccess
{
    class AccessDatabase_OLEDB_JET : IDatabase
    {
        ...
        public override Dictionary<string, 列对象> GetTableColumns(string strTableName)
        {
            if (dic表所有列.Count == 0)
            {
                ...
                foreach (DataRow row in schemaTable.Rows)
                {
                    ...
                    {
                        dtData = GetTableBySQL("select * from [" + strTABLE_NAME + "] where 1=0", true);
                    ...
                    列对象 obj = new 列对象()
                    {
                        ...
                        Is存在默认值 = Convert.ToBoolean(row["COLUMN_HASDEFAULT"]),
                        obj默认值对象 = row["COLUMN_DEFAULT"],
                    };
