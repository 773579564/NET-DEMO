...
namespace DBAccess
{
    class AccessDatabase_ODBC : IDatabase
    {
        private OdbcConnection m_Connection = null;
        private string m_strFileName = null;

        private OdbcCommand command = null;
        ...
        public override void ExecuteSQL(string strSQL, DbTransaction trans = null)
        {
            if (command == null)
            {
                command = new OdbcCommand(null, this.m_Connection);
                command.CommandType = System.Data.CommandType.Text;
            }
            if (command.Transaction != trans)
                command.Transaction = trans as OdbcTransaction;
        ...
        {
            OdbcCommand command = new OdbcCommand(strSQL, this.m_Connection);
        ...
        private OdbcParameter[] GenArrayParamByListObject(Dictionary<string, object> listParam)
        {
            OdbcParameter[] parameterArray = new OdbcParameter[listParam.Count];
            int index = 0;
            foreach (KeyValuePair<string, object> pair in listParam)
            {
                parameterArray[index] = new OdbcParameter(pair.Key, pair.Value);
                index++;
            }
