...
namespace MyHelper
{
    ...
    {
        private int _timeout = 10000;//等待请求开始返回的超时时间.从发出请求开始算起，到与服务器建立连接的时间
        private int _readWriteTimeout = 300000;//等待读取数据完成的超时时间。设置的是从建立连接开始，到下载数据完毕所历经的时间
        private bool _ignoreSSLCheck = true;//是否忽略SSL检查
        private bool _disableWebProxy = false;//是否禁用本地代理
        ...
        /// </summary>
        /// <returns>毫秒数</returns>
        public static long GetCurrentTimeMillis(System.DateTime time)
        {
